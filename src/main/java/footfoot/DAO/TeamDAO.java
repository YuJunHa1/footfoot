...
public class TeamDAO {
	...
	public int create(Team team) {
	    ...
	    try (PreparedStatement pstmt = conn.prepareStatement(SQL, PreparedStatement.RETURN_GENERATED_KEYS)) {
	        // `Team` 객체에서 데이터 가져와 PreparedStatement에 바인딩
	        ...
	        pstmt.setString(6, team.getLeader_id());
	        pstmt.setInt(7, team.getMeeting_day());
	        pstmt.setInt(8, team.getRecruit_position());
	        pstmt.setString(9, team.getTeam_local());

	        // SQL 실행
	        int rowsInserted = pstmt.executeUpdate(); // 삽입된 행의 수 확인
	        if (rowsInserted > 0) {
	            try (ResultSet generatedKeys = pstmt.getGeneratedKeys()) {
	                if (generatedKeys.next()) {
	                    return generatedKeys.getInt(1); // 생성된 team_id 반환
	                }
	            }
	        }
	        return -1; // team_id 가져오기 실패 시 반환 값 설정

	    } catch (SQLException e) {
	        System.err.println("SQL 오류: " + e.getMessage());
	        return -1; // 일반적인 SQL 오류 반환 값 설정

	    } catch (Exception e) {
	        System.err.println("알 수 없는 오류: " + e.getMessage());
	        return -1; // 기타 예외 반환 값 설정
	    }
	}
